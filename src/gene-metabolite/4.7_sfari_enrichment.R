...
# Union of genes that entered >=1 limma model
...
# foreground relative to NON-SFARI genes, with both restricted to `bg`. 

# p is one-sided (greater), matching run_fisher(); the CI comes from the two-sided test 
...
# Every hit gene must be in the tested universe by construction
...
# Test 1 asks "does this score reach ANY association?", 
#Test 2 asks "given an association, does this score reach >= N metabolites?".
strata_all <- bind_rows(strata_t1, strata_t2)
write_csv(strata_all, file.path(out_dir, "sfari_score_stratified_enrichment.csv"))

# ---- Visualizations --------------------------------------------------------

# A0) Score-stratified OR forest, both tests. 
...
# C) Per-cell-type forest. Ordered by OR
...
# D) Score-response: proportion of TESTED SFARI genes at each score that reach at least one association. 
...
# Rank EVERY tested gene by its strongest association (smallest p-value
# across all metabolite x cell-type tests) and walk down the ranking, adding
# when a gene is SFARI and subtracting when it is not. 
...
# ---- Which SFARI genes, in which cell types? ----------------------------
...
if (length(class_col) > 0) {
  ...
  # Keep the classes that matter to either group; collapse the rest 
