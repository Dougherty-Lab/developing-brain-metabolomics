...
# tidyverse verbs loaded here -- select() in particular resolves to a method
# that does not take bare column names and fails with "unused arguments".
# Qualifying at the call site is more durable than reordering the loads.
...
# Minimum distinct hit genes for a cell type to be tested. Below this the
# odds ratio is unstable and the CI spans an order of magnitude; such cell
# types are reported in the CSV with tested = FALSE and greyed out in the
...
# ONE reported set per condition. Where a condition draws on more than one
# publication (SCZ, Epilepsy) the union set is reported; where it draws on one
# (NDD, T2D, IBD) the paper set already IS the condition set. A union and its
# component papers are not independent tests, so reporting both would present
# one result as several. Per-paper tests are still RUN and still written to the
# CSVs as a consistency check -- they are simply not the plotted numbers. Every
# figure in this script filters on this list, so the pooled and per-cell-type
# panels cannot disagree about which sets are on display.
...
# BH is applied WITHIN the reported grid and, separately, within the
# supplementary per-paper grid. Correcting the headline results against tests
# that are never reported -- and that are near-duplicates of them -- would
# inflate their FDR for no inferential gain.
...
# ---- Universes -------------------------------------------------------------
# Global universe is shared with sfari_enrichment.R (same cache file, same
# METHOD key) so the two scripts can never disagree about the background.
universe_cache <- file.path(cache_dir,
                            sprintf("tested_gene_universe_%s.rds", METHOD))
# Per-cell-type universe is specific to Analysis 2.
...
# ---- Gene set coverage -----------------------------------------------------
# A set gene that is not in the tested universe cannot possibly be a hit.
# Restricting each set to the universe is what makes the Fisher table valid;
# coverage is reported because a set with low coverage yields a small effective
# n and a correspondingly unstable OR.
coverage <- gene_sets |>
  group_by(set_id) |>
...
# Symbol-nomenclature warning. Several published sets use older GENCODE
# symbols (WBSCR17 -> GALNT17, PCNXL3 -> PCNX3, FTSJ2 -> MRM2) and clone-based
# lincRNA names (RP11-*). Those fail an exact join and silently shrink the set.
...
# ---- Fisher helper ---------------------------------------------------------
# One-sided (greater): the hypothesis is enrichment, not depletion. The CI is
# taken from the two-sided test because the one-sided interval is [lo, Inf) and
# cannot be plotted. Both p and the BH-adjusted FDR are carried through to the
# output CSVs; significance is judged on FDR only.
...
# The cell-type-agnostic view, at condition level only. Where a condition draws
# on more than one publication (SCZ, Epilepsy) the union set is plotted; where
# it draws on one (NDD, T2D, IBD) the paper set already IS the condition set.
# Per-paper rows are deliberately NOT shown: a union and its components are not
# independent tests, and plotting them together invites reading one result as
# three. The per-paper numbers remain in geneset_enrichment_pooled.csv as a
# consistency check.
#
# Negative controls are faceted apart rather than interleaved: T2D and IBD are
# expected NOT to enrich, and a reader scanning one column of odds ratios would
# otherwise read them as findings.
...
# Fisher's test compares set genes among hits against ALL tested genes. It
# cannot see that hit genes are systematically longer and better expressed than
# background, so ANY curated disease list -- which is inevitably biased toward
# well-studied, well-expressed genes -- can appear enriched for reasons that
# have nothing to do with the disease.
#
# The same control applied to SFARI in sfari_enrichment.R took a Fisher OR of
# 3.80 down to a 1.04-fold excess (p = 0.37), i.e. the entire apparent
# enrichment was detectability. That result makes running the unmatched test
# alone indefensible here: the negative controls exist to detect exactly this
# artefact, and CAD (220 genes) enriches at an odds ratio comparable to the
# neurodevelopmental sets.
#
# For each set, draw N random gene sets of the same size, sampling WITHIN
# strata defined by expression decile x number of cell types in which the gene
# was testable -- the two things that most directly determine whether a gene
# could become a hit at all. If a set's observed overlap with the hit list
# still sits in the tail of that null, the enrichment survives the confound.
N_PERM_MATCH <- 10000
...
# Side-by-side: unmatched odds ratio against matched fold enrichment. The gap
# between the two IS the detectability effect, so plotting them together is
...
# The power check. If enrichment tracks the number of hit genes, the signal is
# detection depth rather than biology. A flat cloud is the reassuring result.
power_df <- per_ct |>
  dplyr::filter(reported, !is.na(OR), is.finite(OR)) |>
...
# The sets are not independent; this is the evidence for saying so.
...
# Everything above returns odds ratios. An OR says enrichment exists; it does
# not say what the enrichment is made of, and the manuscript claims name
# specific cell types (IN-MGE-SST for SCZ and DD, OPC for epilepsy, EN-L6b for
# DD). Those claims need a panel a reader can inspect gene by gene.
#
# Two levels, each computed for the full dataset and then for the highlighted
# cell type x set pairs:
#   gene level       -- which disease-set genes carry associations, where
#   metabolite level -- WHAT they associate with, and whether it differs from
#                       the metabolites associated with non-set genes
#
# Highlights are DERIVED from the results by default rather than hardcoded, so
# the figure tracks the analysis if thresholds or data change. Set
# HIGHLIGHT_OVERRIDE to pin specific pairs instead.
...
# Paper-level neuro sets only. Fill = distinct metabolites, so a reader sees
# both which genes drive the result and whether one cell type carries it.
...
# Do disease-set genes associate with a DIFFERENT KIND of metabolite, or just
# with more of them? A flat comparison is a clean negative worth one sentence.
...
# The specific claims in the Results. One gene-by-metabolite tile plot per
# highlighted pair, so each named enrichment is backed by an inspectable panel.
