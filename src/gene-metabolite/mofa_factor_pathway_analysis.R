...
# Default target: Factor 2, metabolite view.
#
# Three ORA foregrounds per top-N cutoff:
#   1. "all"      — top N metabolites by |weight|
#   2. "positive" — the positively-weighted members of that same top-N set
#   3. "negative" — the negatively-weighted members of that same top-N set
# Plus one signed MSEA (fgsea, scoreType = "std") on the full weight ranking,
# which needs no cutoff and recovers direction natively.
#
# Run from src/gene-metabolite/ (or anywhere inside the repo — paths are
# anchored to the .git root).
#
# Sneha Chaturvedi
# ==============================================================================
...
annot_full <- read_csv(
  file.path(results_dir, "untargeted", "batch2_metabolite_annotations_flagged.csv"),
  show_col_types = FALSE
)
