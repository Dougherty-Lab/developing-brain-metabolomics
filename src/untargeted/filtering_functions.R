...
remove_intensity_outliers <- function(data,
  ...
  if (save_plot) {
    p <- ggplot(diagnostic_df, aes(
      x = reorder(Sample, TotalIntensity),
      y = TotalIntensity,
      color = IsOutlier
    )) +
      geom_point(size = 3) +
      geom_hline(
        yintercept = mean(sample_sums) + threshold * sd(sample_sums),
      ...
      labs(
        title = "Sample Total Intensity Distribution",
...
remove_detection_outliers <- function(data,
  ...
  if (save_plot) {
    p <- ggplot(diagnostic_df, aes(
      x = reorder(Sample, PeaksDetected),
      y = PeaksDetected,
      color = IsOutlier
    )) +
      geom_point(size = 3) +
      geom_hline(
        yintercept = mean(peaks_detected) + threshold * sd(peaks_detected),
        linetype = "dashed", color = "red"
      ) +
      geom_hline(
        yintercept = mean(peaks_detected) - threshold * sd(peaks_detected),
      ...
      theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
      labs(
        title = "Number of Detected Peaks per Sample",
        subtitle = paste0("Z-score threshold: ±", threshold),
        x = "Sample",
  ...
  for (i in seq_len(nrow(column_mode_combinations))) {
    ...
    for (k in 2:(n - 1)) {
      ...
      if (density_smooth_nona[k] <= density_smooth_nona[k - 1] ||
        density_smooth_nona[k] <= density_smooth_nona[k + 1]) {
        next
      }
    ...
    if (!is.na(first_peak_idx)) {
      cat(
        "    First real peak at index:", first_peak_idx,
      ...
      cat(
        "    Valley floor (threshold) at index:", valley_idx,
  ...
  if (verbose) {
    ...
    for (i in seq_len(length(rate_bins))) {
      if (i < length(rate_bins)) {
        ...
        cat(
          "  ", formatC(lower * 100, width = 4, format = "f", digits = 1), "-",
    ...
    if (ncol(dataset_filtered) > 3) {
      ...
      cat(
        "  Range:", round(min(sample_detection) * 100, 2), "-",
...
plot_missingness_distribution <- function(
  ...
  p <- ggplot(missingness_stats, aes(x = detection_rate * 100, fill = detection_rate >= detection_threshold)) +
    ...
    labs(
      ...
      subtitle = paste0(
        "Threshold: ≥", detection_threshold * 100, "% | ",
