...
# ---------------------------------------------------------------------------
# MOFA factor values and key gene expression overlaid on UMAP embeddings.
#
# Inputs:  results/mofa/mofa_model.hdf5, data/cache/SSD_meta_umap.rds
# Outputs: PNG + SVG in results/mofa/umap-exploration/
#
# Upstream:  mofa_integration.Rmd, prep_data.R
# Downstream: None
# ---------------------------------------------------------------------------
...
out_dir      <- file.path(root, "results", "mofa", "umap-exploration")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

source(file.path(root, "src/gene-hormone/pseudobulk_functions.R"))

# ---- Set your MOFA model path ----------------------------------------
mofa_path <- file.path(root, "results", "mofa", "mofa_model.hdf5")
...
if (!has_expr) {
  ...
  seurat_full_path <- file.path(root, "data", "SSD_RNAonly_filtered.rds")
